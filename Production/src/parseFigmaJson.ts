...
function BladeColorConverter(color: string, primaryColor: string): string{
  if(color == primaryColor) return "surface.background.primary.normal";
  if(color == 	"#f0f4fa") return "surface.background.gray.subtle"
  else if(color == "#f0f4fa") return "surface.background.gray.moderate"
  else if(color == "#ffffff") return "surface.background.gray.intense"
  else if(color == "#00a352") return "feedback.background.positive.intense"
  else if(color == "	#d92d20") return "feedback.background.negative.intense"
  return "surface.background.gray.subtle";
}

function analyzeLayout(parent: simpleNodeInterface): {
  ...
  if(ChildrenatMinLeft ==1){
    ...
    let alignItems: "flex-start" | "center" | "flex-end" | "stretch" | undefined = undefined;
    ...
    let justifyContent: "flex-start" | "center" | "flex-end" | "space-between" | "space-around" | "space-evenly" | undefined = undefined;
...
function simplifyNode(node:node) {
  ...
  if(node.name == "Browsers [Utility]") return null;
  if(node.type == "VECTOR"){
    return null;
  }
  ...
  if (node.children && node.children.length > 0) {
    let children = node.children
      ...
      .map(simplifyNode)
      .filter((child): child is simpleNodeInterface => (child !== null));
...
let PrimaryColor = "";
async function parseFigmaJson(figmaData: any, Log: string, primaryColor: string){
  ...
  try{
    ...
    fs.writeFileSync(`${process.env.cwd}Outputs/Layout.json`, JSON.stringify(simplifiedTree, null, 2));
